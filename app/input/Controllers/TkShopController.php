...
class TkShopController extends Controller
{
    protected function _productAsText($product) {
        ...
        $productReduced=$this->_objectReduceToKeys($product, $keys);
        foreach ($productReduced as $key => $value) {      
...
//      $mail->Password = 'F66gurkburk04';                           // SMTP password
        ...
        $mail->setFrom('webmaster@nyasidan.tangokompaniet.com', 'nyasidan.tangokompaniet.com');
        $mail->addAddress('per.eskilson@gmail.com');
        $mail->addReplyTo('webmaster@nyasidan.tangokompaniet.com', 'order@tangokompaniet.com');
    ...
    protected function _insertProductDef($insertObj)
    {
        ...
        $sql=$this->_build_sql_insert($table, $insertObj);
        if ($this->_sqlInsert($sql)) {
            $this->logger->error('OK: Successful insert of productId = ' . $insertObj['newProductId'] . ' into table tbl_product_def');
        } else {
    ...
    protected function _insert($table, $rec) 
    {
        try {
            $con = $this->db;
            $sql = $this->build_sql_insert($table, $rec);
            $sth = $con->prepare($sql);
            $this->_sqlInsert($sql);
        ...
        // Get the id of the latest 978
        $insertId = 122;
        $this->logger->addDebug('TkShopController._insert: insertId:' . $insertId);
        return $insertId; 
    }    
    ...
    protected function _insertOrderLine($table, $order, $requiredKeys, $reduceToKeys) {
        if (($str = $this->_checkMandatoryInput($order, $requiredKeys))===null) {
            ...
            $sql = $this->_build_sql_insert($table, $obj);
            if (!$this->_sqlInsert($sql)) {
    ...
    {
        $con = $this->db;
        $obj = $this->_objectReduceToKeys($order, ['amount', 'status', 'eventType']); 
        if (!isset($order['status'])) {
            $obj += ['status'=>'OK'];
        }
        $sql = $this->_build_sql_insert('tbl_order', $obj);
    ...
    protected function UNUSED_insertRegistrationIntoTable($table, $reg, $cols) 
    {
        $regReduced = $this->_objectReduceToKeys($reg, $cols);
        $sql = $this->_build_sql_insert($table, $regReduced);
        $this->logger->addDebug('SQL-statement:' . $sql);

        return($this->_sqlInsert($sql));
    }
    ...
    protected function _insertSingleReg($reg) 
    {
        ...
        $cols=['productType', 'status', 'productId', 'leader', 'firstName', 'lastName', 'email', 'phone', 'firstNamePartner', 'lastNamePartner', 'emailPartner', 'message', 'orderId'];
        if (isset($reg['phonePartner'])) {
            $cols[]='phonePartner';
        }     
        if (isset($reg['label'])) {
        ...
        $this->logger->addDebug('before _replaceRowInTable for registrator:' . TBL_REGISTRATION);
        if (!$this->_replaceRowInTable('tbl_registration', $reducedReg)) {
            return false;
        } 
        ...
        if (isset($reducedReg['firstNamePartner']) && isset($reducedReg['lastNamePartner'])) {
            ...
            /*
            $this->logger->addDebug('_insertSingleReg: before _replaceRowInTable for reducedRegPartner');
            if (!$this->_replaceRowInTable(TBL_REGISTRATION, $reducedRegPartner)) {
                return false;
            } 
            */   
    ...
    protected function _insertExtendedProductList($table, $extendedProductList) 
    {
        $returnValue = false;

        foreach ($extendedProductList as $product) {
            $productReduced = array();

            foreach ($product as $key=>$value) {
                if (filter_var($value, FILTER_VALIDATE_EMAIL)) {
                    $productReduced[$key] = strtolower($value);
                } else {   
                    $productReduced[$key] = $value;
                }
            } 
            if (isset($productReduced['year'])) {
                unset($productReduced['year']);
            }
    
            /* Insert or concat a registration */
            if (!$this->_replaceRowInTable($table, $productReduced)) {
    ...
    protected function _insertEventReg($table, $reg) 
    {
        ...
        if (isset($reg['dancePartner'])) {
            $dancePartner=true;
            unset($reg['dancePartner']);
        }
        $regReduced = array();
        foreach ($reg as $key=>$value) {
            $newValue = is_array($value)?implode(', ', $value ):$value;
            $this->logger->addDebug("_insertEventReg: $newValue");
            if (filter_var($value, FILTER_VALIDATE_EMAIL)) {
                $regReduced[$key] = strtolower($newValue);
            } else {   
                $regReduced[$key] = $newValue;
            }
        } 
        ...
        /* Insert or concat a registration */
        if (!$this->_replaceRowInTable($table, $regReduced)) {
            $this->logger->error('Failed go insert or concat into ' . $table);
            return false;
        }    
        // Insert into phonebook
    ...
    protected function _insertAllReg($orderId, $regs) {
        $nbrReg=count($regs);
    ...
    protected function _insertRegistrations($orderId, $groupByArr) {
        ...
        if (isset($groupByArr['workshop'])) {
            $regs = array_merge($regs, $groupByArr['workshop']);
        }    
        if (isset($groupByArr['marathon'])) {
            $regs = array_merge($regs, $groupByArr['marathon']);
        }
        if (isset($groupByArr['social'])) {
            $regs = array_merge($regs, $groupByArr['social']);
        }
        return $this->_insertAllReg($orderId, $regs);
    }
    ...
    protected function _createOrder($customer, $shoppingCartList, $currency, $language) 
    {
        ...
        $regObj = $this->_insertRegistrations($orderId, $groupByArr);
    ...
    public function createRegistration($request, $response) 
    {
        ...
        if (isset($data['registration'])) {
            $reg = $data['registration'];  // Old way of encaspsulating. Changed 21/9-2000
        } else {
            $reg = $data;    
        }
    ...
    protected function _sendMailEventConfirmation($reg) 
    {
        $arr = array('email', 'firstName', 'emailResponsible', 'year', 'eventType');
        foreach ($arr as $key => $value) {
            if (!isset($reg[$value])) {
                $this->logger->error("ERROR in _sendMailEventConfirmation: Failed to find key $value");
                return false;
            }   
        }
        ...
        $body="<h1>Malmö Tango Marathon $year registration </h1>"; 
        $body ="<div style='color:#2b2523;'>";
        $body .= $this->_getHtmlFromTblText($eventType, 'MAILBODY', $language);
        $body .= "<h3>Details of your request</h3>";
        //$body .= $this->_getHtmlFromTblText('MARATHON', 'MAILBODY', $language);
        $body .= $this->_build_html_from_object($reg);
        ...
        if ($this->_sendMail($subject, $body, $emailRepsonsible)) {         
            $this->logger->addDebug('Successful sending of mail to ' . $emailRepsonsible);
            $this->logger->addDebug('Body ' . $body);
            return false;
        } else{
    ...
    public function createRegistrationMarathon($request, $response) 
    {
        ...
        if (($ret=$this->_checkMandatoryInput($reg, ['firstName', 'lastName', 'email', 'emailResponsible', 'year', 'eventType'])) !== null) {
            return $response->withJson(array('status'=>'ERROR',
        ...
        $this->logger->addDebug('In createRegistrationMarathon');

        // Insert order into tbl_order and get in return orderId
        if ($this->_insertEventReg(TBL_REGISTRATION_MARATHON, $reg)) {
            $mailReturnValue = $this->_sendMailEventConfirmation($reg);
            $this->logger->info('createRegistrationMarathon: success');
            return $response->withJson(array('status'=>'OK', 
                    'orderId'=>$orderId,
                    'mail'=>$mailReturnValue,
            ...
            if (($mailReturnValue=$this->_sendMail($subject, $body, $email))===true) {         
    ...
    public function createRegistrationFestival($request, $response) 
    {
        ...
        if (!$this->_insertExtendedProductList(TBL_REGISTRATION_FESTIVAL_PRODUCT, $reg['extendedProductList'])) {
        ...
        if (isset($reg['extendedProductList'])) {
            unset($reg['extendedProductList']);
        }

        // Insert order into tbl_order and get in return orderId
        if ($this->_insertEventReg(TBL_REGISTRATION_FESTIVAL, $reg)) {
            $this->_sendMailEventConfirmation($reg);
            $this->logger->addDebug("createRegistrationFestival: successful insert of $eventType registration for orderId=$orderId");
            return $response->withJson(array(
                'status'=>'OK', 
                'orderId'=> $orderId, 
            ...
            $this->logger->error('createRegistrationFestival: FAILED');
            $subject="Registration to $eventType FAILED";
            $body="<p>Please contact tangokompaniet and make registration manually</p>";    
            if ($this->_sendMail($subject, $body, $email)) {         
                $this->logger->addDebug('Successful mail sending to ' . $email);
                $this->logger->addDebug('Body ' . $body);
            ...
            return $response->withJson(array(
                'status'=>'ERROR',
                'orderId'=> $orderId, 
                'message'=>"Failed to make registration on $eventType for $firstName $lastName $email to marathon"), 204);
        } 
    }
