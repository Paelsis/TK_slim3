...
class TkMailController extends Controller
{
    ...
    protected function _echoPaymentInfo($orderId, $language) {
        TEXT['ORDER_ID_TEXT'][$language];
        echo "<ul>";
            echo "<li>SWISH:" . SWISH . TEXT['ONLY_IN_SWEDEN'][$language] . "</li>";
            echo "<li>Bankgiro:" . BANKGIRO . TEXT['ONLY_IN_SWEDEN'][$language] . "</li>";
    ...
    protected function _mailBodyToCustomer($reg) 
    {
        ...
        $status = TEXT['STATUS'][$reg['status']][$language]; 

        $header ="<h1>" . TEXT['REGISTRATION'][$language] . $orderId . "</h1>";

        $part1 = TEXT['EVENT_NAME'][$language] . ' : ' . $name . "<p/><p/>";
        
        $part2=TEXT['TIME'][$language] . $weekday . ' ' . $reg['startDate'] .  ' at ' . $reg['startTime'] . '<p/><p/>';
        ...
        $part5=isset($reg['price'])?'<p/><p/>' . TEXT['PRICE_PER_PERSON'][$language] . ' : ' . $reg['price'] . ' ' . $currency . ' (shall be paid to Tangokompaniet with one of the methods below)<p/>><p/>':'';
        ...
        $this->_echoPaymentInfo($orderId, $language);
        echo '<h4>' . TEXT['THANK_YOU'][$language] . '</h4>';
        $part7 = ob_get_contents(); //Grab output
        ob_end_clean(); //Discard output buffer

        $this->logger->warning("77777");


        $body = "<div style='color:#81185B;'>" . $header . $part1 .  $part2 . $part3 . $part4 . $part5 . $part6 . $part7 . '</div>';
        $body .= isset($reg['message'])?TEXT['MESSAGE_SENT_TO_TK'][$language] . $reg['message']:'';
        return $body;
    }
