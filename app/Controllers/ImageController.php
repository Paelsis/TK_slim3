...
class ImageController extends Controller
{
    ...
    protected function _rotateFile($path, $file, $angle) 
    {
        ...
        if (exif_imagetype ($fileName) === IMAGETYPE_PNG) {
            ...
            if ($source !== false) {
                ...
                $this->logger->error('File could not create an image source from .png-file, filename= ' . $fileName);
                return false;
            }    
            ...
            if ($source !== false) {
                ...
                $this->logger->error('File could not create an image source from .jpg-file, filename= ' . $fileName);
                return false;
            }    
        } else {
            $this->logger->error('Filetype is wrong and cannot to rotate, filename= ' . $fileName);
            return false;
        }
    }    
    ...
    protected function _imagePath($payload) {
        ...
        if (isset($payload['subdir'])) {
            if (isset($payload['rootdir'])) {
                if (($payload['rootdir'] !== '.') && ($payload['rootdir'] !== null)) {
    ...
    public function deleteDirectory($request, $response) 
    {
        ...
        $orig = $path;
        $base = basename($orig);
        $dest = USER_ROOT_DIR . '/' . '__trashcan/' . $base . '.' . $suffix;
    ...
    public function setImageList($request, $response) 
    {
        $parsedBody = $request->getParsedBody();
        $uid = isset($queryParams['uid'])?$queryParams['uid']:'No uid';
        $payload = isset($parsedBody['payload'])?$parsedBody['payload']:array();
        ...
        $json = json_encode(array('payload' => $payload));

        //write json to file
        $fname = isset($uid)?"../public/$uid.data.json":"../public/data.json";
        if (file_put_contents($fname, $json))
            return $response->withJson(array('status' => 'OK', 'result'=>$json) ,200);
        else 
            return $response->withJson(array('status' => 'ERROR', 'message'=>'Failed to write to ' . $fname, 'fname'=>$fname, 'result'=>$json) ,200);
    }    
    ...
    public function getImageList($request, $response) 
    {
        ...
        $fname = isset($queryParams['uid'])?$publicDir . '/' . "$uid.data.json":$publicDir . "data.json";
        $string = file_get_contents($fname);
        if ($string === false) {
            return $response->withJson(array('status' => 'ERROR', 'message'=>'Failed to read data from ' . $fname, 'fname'=>$fname) ,200);
        }       

        //Decode JSON
        $json_data = json_decode($string,true);
        return $response->withJson(array('status' => 'OK', 'result'=>$json_data) ,200);
    }    

    public function setJsonInFile($request, $response) 
    {
        $payload = $request->getParsedBody();
        $array = isset($payload['array'])?$payload['array']:array(); 
        $fname = isset($payload['fname'])?$payload['fname']:'images.json';

        $this->logger->info("setJsonInFile fname = $fname");


        // Set and check path    
        $publicDir = __DIR__ . "/../../public/";
        $path = $publicDir . $fname;
        if (!file_exists($path)) {
            return $response->withJson(array('status' => 'ERROR', 'message'=>'File ' . $fname) ,200);
        }

        // Encode array to json
        $json = json_encode($array);

        file_put_contents($path.date('.ymd:His'), $json);

        // write json to file $fname
        if (file_put_contents($path, $json)) {
            return $response->withJson(array('status' => 'OK', 'json'=>$json) ,200);
        } else {
            return $response->withJson(array('status' => 'ERROR', 'json'=>$json, 'fname'=>$fname) ,200);
        }    
    }    

    public function getJsonFromFile($request, $response) 
    {
        $queryParams = $request->getQueryParams();
        $fname = isset($queryParams['fname'])?$queryParams['fname']:'images.json';
        $publicDir = __DIR__ . "/../../public/";
        $path = $publicDir . $fname;
        ...
        if ($result != null) {
            ...
            return $response->withJson(array('status' => 'ERROR', 'fname'=>$fname, 'json'=>$strJsonFileContents), 200);
        }    
    }    
} 
