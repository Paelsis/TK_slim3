...
class TkImageController extends Controller
{
    protected function _rotateFile($path, $file, $angle) 
    {
        ...
        if (!file_exists($fileName))  {
            $this->logger->error("_rotateFile: File $fileName does not exist");
            return false;
            ...
            if ($source !== false) {
                ...
                imagedestroy($source);
                $this->logger->info('_rotateFile: Successful rotate of png ' . $path . '/' . $file . ' with ' . $angle . ' degrees');
                return true;
            } else {
                $this->logger->error('_rotateFile: File could not create an image source from .png-file, filename= ' . $fileName);
                return false;
            }    
            ...
            if ($source !== false) {
                ...
                imagedestroy($source);
                $this->logger->info('_rotateFile: Successful rotate of jpg ' . $path . '/' . $file . ' with ' . $angle . ' degrees');
                return true;
            } else {
                $this->logger->error('_rotateFile: File could not create an image source from .jpg-file, filename= ' . $fileName);
                return false;
            }    
        } else {
            $this->logger->error('Wrong filetype for image:' . $fileName);
            return false;
        }
    }    
        ...
        if (isset($file['delete'])) {
            if (isset($file['fname'])) {
                if ($this->_removeFile($subdir, $file['fname'])) {
                    ...
                    $this->logger->warning("_removeFileAndThumb: Failed to remove file " . $file['fname']);
                };
            }    
            if (isset($file['thumbFname'])) {
                if ($this->_removeFile($subdir, $file['thumbFname'])) {
                    ...
                    $this->logger->warning("_removeFileAndThumb: Failed to remove thumb " . $file['thumbFname']);
                };
            }    
        }    
        return($cnt);
    ...
    public function removeOrRotateImages($request, $response) 
    {
        ...
        $list = $this->_listImagesData($path);
        if (($numberOfDeletedFiles + $numberOfRotatedFiles) > 0) {
            return $response->withJson(
                array('status' => 'OK',
                'deleted'=>$numberOfDeletedFiles,
                'rotated'=>$numberOfRotatedFiles,
                'result'=>$list), 200);
        } else {
            return $response->withJson(
    ...
    public function listImages($request, $response) 
    {
        ...
        if ($handle) {
            while (false !== ($entry = readdir($handle))) {
                if ($entry != "." && $entry != ".." && $this->_isImage($entry) && !$this->_isThumbnailAndOriginal($entry))  {
                    $result[] = $entry;
                }
            }
            closedir($handle);
                ...
                if ($entry != "." && $entry != ".." && $this->_isImage($entry) && !$this->_isThumbnail($entry))  {
                    ...
                    */
                    ...
                    if (file_exists($path . '/' . $thumbFname)) {
                        $image += array('thumbFname'=>$thumbFname, 'fname_thumb'=>$thumbFname); 
                    }
                    $list[] = $image;
                } 
            }
    ...
    public function postImages($request, $response) 
    {
        ...
        return $response->withJson(
            array(
                ...
                'count'=>$cnt, 
                'path'=>$path, 
                'fullpath'=>'https://tangokompaniet.com/app/slim/public/images/', 
                'result'=> $list,
                'message'=>'Image/s upladed')
            , 200);
    }
