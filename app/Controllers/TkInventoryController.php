    ...
    protected function _updateProductInv($productId, $inventoryList) 
    {
        if (count($inventoryList) > 0) {
            ...
            foreach($inventoryList as $inv) {
                ...
                $duplet = $this->_build_sql_insert_duplet($table, $reducedObject);
                if ($this->_sqlInsert($duplet['sql'], $duplet['values'])) {
                    $this->logger->error('OK: Successful insert to tbl_product_inv');
                } else {
                    $this->logger->error('ERROR: Could not insert ibti tbl_product_inv');
                }
            }
        }
        return(true);
