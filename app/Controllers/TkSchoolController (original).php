...
class TkSchoolController extends Controller
{
    ...
    public function getUpdateTable($request, $response) 
    {
        ...
        if (($str = $this->_checkMandatoryInput($allGetVars, ['id', 'presence']))!= null) {
            return
            $response->withJson(array('message' => $str, 'result'=>null),422);
        }
    ...
    public function updateRowsInPresence($request, $response) 
    {
        ...
        if (count($rows) === 0) {
            ...
            return $response->withJson(array('status' => 'OK', 'message'=>'Succesful update of rows', 'result'=>$rows, 'count'=>count($rows)) ,200);
        } else{
    ...
    public function replaceRow($request, $response) 
    {
        ...
        if (($str = $this->_checkMandatoryInput($input, ['table', 'data'])) !== null) {
            return $response->withJson(array('replaceRow: status'=>'WARNING', 'message' => $str . 'in table ' . $table , 'result'=>null),200);
        }    
        ...
        if ($this->_replaceRow($table, $row)) {
            ...
            return $response->withJson(array('status' => 'OK', 'message'=> 'successful', 'result'=>$row) ,200);
        } else{
    ...
    protected function _getRegistrationCourseSQL($language)
    {
        ...
        P.price,
        IFNULL(concat(IFNULL(t1.firstName, null), IF (t2.firstName is null, '', ' & '), IFNULL(t2.firstName,null)), '?') as teachers, 
        IFNULL(concat(IFNULL(t1.shortName, null), IF (t2.shortName is null, '', ' & '), IFNULL(t2.shortName,null)), '?') as teachersShort,
        ...
        and DATE(SUBSTR(R.productId, 1,6)) >= DATE_SUB(NOW(), INTERVAL 52
    ...
    protected function _getTeacherNoteSQL($tableName, $language)
    {
        return(" 
        SELECT T.id, 
        T.creaTimestamp, 
        ...
        S.siteName,
        S.city,
        CONCAT(IFNULL(ed.name$language, IF(SUBSTR(T.textId, 3,4) < '07', 'Spring JAN-JUN', 'Autumn JUL-DEC')),' ', IFNULL(C.year, CONCAT(20, SUBSTR(T.textId, 1,2)))) as scheduleName,
        CONCAT(IFNULL(ed.name$language, IF(SUBSTR(T.textId, 3,4) < '07', 'Spring JAN-JUN', 'Autumn JUL-DEC')),' ', IFNULL(C.year, CONCAT(20, SUBSTR(T.textId, 1,2)))) as scheduleId,
        IFNULL(D.name$language, SUBSTR(T.textId, 13, char_length(T.textId) -22)) as courseName,
        DATE_FORMAT(
            STR_TO_DATE(
                SUBSTR(SUBSTRING(T.textId, 1, char_length(T.textId) -10), 1, 6),
                '%y%m%d'
            ),
            '%d%b%y'
        ) AS startDate,
        TIME_FORMAT(STR_TO_DATE(SUBSTR(SUBSTR(T.textId, 1, char_length(T.textId) -10) , 7, 4), '%H%i'), '%H:%i') as startTime,
        date_format(right(T.textId, 10), '%W %d %b %Y') as courseDateForDisplay,
        right(T.textId, 10) as courseDate,
        IFNULL(dayname(right(T.textId, 10)), null) as dayname,
        substr(T.textId, 7,4) as time, 
        SUBSTRING(T.textId, 1, char_length(T.textId) -10) as productId, 
        language, textBody 
        from `$tableName` as T
        left outer join `tbl_course` as C on C.productId = SUBSTRING(T.textId, 1, char_length(T.textId) -10) 
        left outer join `tbl_site` S on S.siteId = SUBSTR(T.textId, 11,2)
        left outer join `tbl_course_def` D on D.courseId = C.courseId
        left outer join tbl_semester_def ed on ed.eventType = IFNULL(C.eventType, IF(SUBSTR(T.textId, 3,4) < '07', 'SPRING', 'AUTUMN'))
        order by D.sequenceNumber, WEEKDAY(C.startDate), productId, startDate"
        );
    }
    ...
    public function getPresenceHistoryMatrix($request, $response) 
    {
        ...
        $emailArray = array();
        foreach ($rows as $row) {
            $emailArray[$row['productId']][$row['email']]=$row;
        }
        ...
        foreach($productIdArray as $productId) {
            foreach($emailArray[$productId] as $email=>$row) {
                foreach($courseDateArray[$productId] as $courseDateDDMM=>$dummy) {
                    if (isset($distMatrix[$productId][$email][$courseDateDDMM])) {
                        $fullMatrix[$productId][$email][$courseDateDDMM] = $distMatrix[$productId][$email][$courseDateDDMM];
        ...
        if (isset($fullMatrix)) {
            return $response->withJson(array('status' => 'true','result'=>$fullMatrix) ,200);
        } else{
            return $response->withJson(array('status' => 'No entries found in database'),422);
        }
    }
    ...
    public function getTeacherNote($request, $response) 
    {
        ...
        $sql=$this->_getTeacherNoteSQL($tableName, $language);
        $rows = $this->_fetchRows($sql); 
    ...
    public function cancelRegistration($request, $response) 
    {
        ...
        if ($rowCount > 0) {
            ...
            return $response->withJson(array('status' => 'OK',"message"=>"Successful cancellation of registration. $rowCount rows deleted in database.") ,200);
        } else {    
            return $response->withJson(array('status' => 'OK',"message"=>"Cancellation can not be done due to date restrictions (or has already been executed).") ,200);
        }    
    }         
};    
